...
@dataclass
class Config:
    eeg_root: Path = Path("eeg_data/EEG")
    video_root: Path = Path("video_data/Video")
...
class VideoFeatureExtractor:
    ...
    def extract_clip_embedding(self, video_path: Path) -> torch.Tensor:
        ...
        batch = torch.stack([self.preprocess(fr) for fr in frames]).to(self.device)
...
class ClipScorer:
    ...
    def embed_frames(self, frames: List[torch.Tensor]) -> torch.Tensor:
        ...
        batch = torch.stack([self.preprocess(im) for im in pil_frames]).to(self.device)
